...
require __DIR__.'/class/dolsarbacane.class.php';
...
{
	$sql = "SELECT m.rowid, m.titre, m.nbemail, m.statut, m.date_creat as datec, m.date_envoi as date_envoi, ds.rowid as fk_dolsarbacane";
	$sql .= " FROM ".MAIN_DB_PREFIX."mailing as m";
	$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."sarbacane as ds on (ds.fk_mailing = m.rowid)";
...
if ($resql)
{
	...
	print_liste_field_titre("SarbacaneCampaign", $_SERVER["PHP_SELF"], "ds.rowid", $param, "", 'align="center"', $sortfield, $sortorder);
	...
	while ($i < min($num, $limit))
	{
		...
		$res = $sarbacane->fetch($obj->fk_dolsarbacane);
		print '<td class="center">';
		if($res > 0) {
            print $sarbacane->getNomUrl();
            if(! empty($sarbacane->sarbacane_id)) print ' <br/> '.$sarbacane->getExternalNomUrl();
        }
		 print '</td>';
		// Date creation
