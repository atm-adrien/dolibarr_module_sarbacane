...
$hookmanager->initHooks(array('sarbacanelist'));
...
llxHeader('', $langs->trans('sarbacaneList'), '', '');

//$type = GETPOST('type');
//if (empty($user->rights->sarbacane->all->read)) $type = 'mine';
...
$sql.= ' FROM '.MAIN_DB_PREFIX.'sarbacane t ';

if (!empty($object->isextrafieldmanaged))
{
    $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'sarbacane_extrafields et ON (et.fk_object = t.rowid)';
}

$sql.= ' WHERE 1=1';
//$sql.= ' AND t.entity IN ('.getEntity('sarbacane', 1).')';
...
$formcore = new TFormCore($_SERVER['PHP_SELF'], 'form_list_sarbacane', 'GET');
...
$listViewConfig = array(
	...
	,'list' => array(
		...
		,'picto_suivant' => '>'
		,'noheader' => 0
		,'messageNothing' => $langs->trans('Nosarbacane')
	...
	,'search' => array(
		...
		,'status' => array('search_type' => sarbacane::$TStatus, 'to_translate' => true) // select html, la clé = le status de l'objet, 'to_translate' à true si nécessaire
	)
	,'translate' => array()
	,'hide' => array(
