...
class sarbacane extends SeedObject
{
    /**
     * Canceled status
     */
	...
	public static $TStatus = array(
		self::STATUS_CANCELED => 'sarbacaneStatusShortCanceled'
		,self::STATUS_DRAFT => 'sarbacaneStatusShortDraft'
		,self::STATUS_VALIDATED => 'sarbacaneStatusShortValidated'
//		,self::STATUS_REFUSED => 'sarbacaneStatusShortRefused'
//		,self::STATUS_ACCEPTED => 'sarbacaneStatusShortAccepted'
	);

	/** @var string $table_element Table name in SQL */
	public $table_element = 'sarbacane';

	/** @var string $element Name of the element (tip for better integration in Dolibarr: this value should be the reflection of the class name with ucfirst() function) */
	public $element = 'sarbacane';
...
//        ),
    ...
    private function getNextRef()
    {
		...
		$mask = !empty($conf->global->SARBACANE_REF_MASK) ? $conf->global->SARBACANE_REF_MASK : 'MM{yy}{mm}-{0000}';
		$ref = get_next_value($db, $mask, 'sarbacane', 'ref');
...
//            $this->fk_user_valid = $user->id;
    ...
    public function getNomUrl($withpicto = 0, $moreparams = '')
    {
        ...
        $result='';
        $label = '<u>' . $langs->trans("Showsarbacane") . '</u>';
        if (! empty($this->ref)) $label.= '<br><b>'.$langs->trans('Ref').':</b> '.$this->ref;

        $linkclose = '" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $link = '<a href="'.dol_buildpath('/sarbacane/card.php', 1).'?id='.$this->id.urlencode($moreparams).$linkclose;
...
//        $picto='sarbacane@sarbacane';
    ...
    public static function LibStatut($status, $mode)
    {
        ...
        if ($status==self::STATUS_CANCELED) { $statusType='status9'; $statusLabel=$langs->trans('sarbacaneStatusCancel'); $statusLabelShort=$langs->trans('sarbacaneStatusShortCancel'); }
        elseif ($status==self::STATUS_DRAFT) { $statusType='status0'; $statusLabel=$langs->trans('sarbacaneStatusDraft'); $statusLabelShort=$langs->trans('sarbacaneStatusShortDraft'); }
        elseif ($status==self::STATUS_VALIDATED) { $statusType='status1'; $statusLabel=$langs->trans('sarbacaneStatusValidated'); $statusLabelShort=$langs->trans('sarbacaneStatusShortValidate'); }
        elseif ($status==self::STATUS_REFUSED) { $statusType='status5'; $statusLabel=$langs->trans('sarbacaneStatusRefused'); $statusLabelShort=$langs->trans('sarbacaneStatusShortRefused'); }
        elseif ($status==self::STATUS_ACCEPTED) { $statusType='status6'; $statusLabel=$langs->trans('sarbacaneStatusAccepted'); $statusLabelShort=$langs->trans('sarbacaneStatusShortAccepted'); }
...
//class sarbacaneDet extends SeedObject
//{
//    public $table_element = 'sarbacanedet';
//
//    public $element = 'sarbacanedet';
//
//
//    /**
//     * sarbacaneDet constructor.
//     * @param DoliDB    $db    Database connector
//     */
//    public function __construct($db)
//    {
